<template>
  <div>
    <header class="city-head">
      ...
      <span class="head-city">{{thiscity.name}}</span>
      <a @click.prevent="$router.go(-1)" class="head-login">切换城市</a>
    </header>
    <form @submit.prevent="handleSearch" autocomplete="off" method="post" class="city_form">
      <div>
        <input v-model="address.name" type="text" name="name" placeholder="输入学校、商务楼、地址" required="required"
    ...
    <ul class="getpois_ul">
      <router-link @click.native="addCityName(option.name)" :to="{name:'shoplist',query:{name:option.name}}" tag="li"
        v-for="(option,index) in this.alladdress " :key="index">
...
<script>
  export default {
    ...
    beforeRouteEnter(to, from, next) {
      next(vm => vm.getId(to.params.id));
    },
    methods: {
      getThisCity(id) {
        this.id = id;
        this.$axios
          .get("https://elm.cangdu.org/v1/cities/" + id)
          .then(res => {
